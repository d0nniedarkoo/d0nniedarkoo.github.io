...
const deployment = 'portfolio-2026-v9';
...
document.body.insertAdjacentHTML('afterbegin', `
  ...
  ${current === 'index.html' ? `<div class="camera-intro" aria-hidden="true"><div class="camera-grid"></div><div class="studio-shadow"></div><div class="lens-stack"><div class="lens-ring ring-one"></div><div class="lens-ring ring-two"></div><div class="lens-ring ring-three"></div><div class="aperture">${Array.from({ length: 8 }, (_, index) => `<i style="--blade:${index}"></i>`).join('')}</div><div class="lens-core"></div><div class="focus-reticle"><span></span><span></span><span></span><span></span></div></div><p>THE ARCHIVE / OPTICAL SYSTEM</p></div>` : ''}`);
...
document.querySelectorAll('.gallery').forEach(gallery => {
  ...
  gallery.insertAdjacentHTML('afterend', `<div class="gallery-ui"><div class="gallery-controls"><button class="gallery-prev" aria-label="Previous image">Prev</button><span aria-live="polite">${firstLabel}1 / ${images.length}</span><button class="gallery-next" aria-label="Next image">Next</button></div><div class="gallery-groups" aria-label="Project folders">${groups.map((group, index) => `<button class="${index === 0 ? 'active' : ''}" data-group="${group}">${group}</button>`).join('')}</div><div class="gallery-thumbs" aria-label="Gallery thumbnails">${images.map((image, index) => `<button class="${index === 0 ? 'active' : ''}" aria-label="View ${image.dataset.group || 'image'} ${index + 1}"><img loading="lazy" src="${image.getAttribute('src')}" alt=""></button>`).join('')}</div></div>`);

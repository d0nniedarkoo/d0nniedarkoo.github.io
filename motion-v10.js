...
const deployment = 'portfolio-2026-v10';
...
document.body.insertAdjacentHTML('afterbegin', `
  ...
  ${current === 'index.html' ? `<div class="camera-intro" aria-hidden="true"><div class="camera-blueprint"></div><div class="camera-assembly"><div class="camera-part viewfinder"><i></i></div><div class="camera-part top-plate"><i></i><i></i><i></i></div><div class="camera-part rear-shell"><span></span></div><div class="camera-part processor-board"><i></i><i></i><i></i><i></i></div><div class="camera-part sensor-unit"><i></i></div><div class="camera-part shutter-unit">${Array.from({ length: 6 }, (_, index) => `<i style="--slat:${index}"></i>`).join('')}</div><div class="camera-part mirror-box"></div><div class="camera-part main-body"><i class="grip"></i><i class="mount"></i><i class="badge"></i><span class="body-screw s1"></span><span class="body-screw s2"></span><span class="body-screw s3"></span></div><div class="camera-part lens-barrel"><i class="glass"></i><i class="focus-ring"></i><i class="aperture-ring"></i></div><div class="camera-part front-ring"></div><div class="camera-part battery"></div></div><div class="camera-floor-shadow"></div><p>THE ARCHIVE / CAMERA SYSTEM 01</p></div>` : ''}`);
...
const archiveCatalog = (folder, prefix, groups) => {
  ...
  return groups.flatMap(([group, count]) => Array.from({ length: count }, (_, groupIndex) => {
    ...
    return {
      group,
      slug: projectSlug(group),
      src: `media/archive/${folder}/${prefix}-${number}.jpg`,
...
if (projectPage) {
  ...
  const groups = projectPage.groups.map(([label, count]) => ({ label, count, slug: projectSlug(label), first: catalog.find(image => image.group === label) }));
...
if (projectPage && selectedProject) {
  const projects = projectPage.groups.map(([label]) => ({ label, slug: projectSlug(label) }));

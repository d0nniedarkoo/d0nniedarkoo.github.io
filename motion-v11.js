...
const deployment = 'portfolio-2026-v11';
...
const archiveGroups = {
  color: [['Project 1', 6, 'project-1'], ['Project 4', 9, 'project-4'], ['Project 5', 3, 'project-5'], ['Project 23', 4, 'project-23'], ['Project 24', 4, 'project-24'], ['Project 25', 8, 'project-25'], ['Project 26', 4, 'project-26'], ['Project 27', 9, 'project-27'], ['Project 28', 2, 'project-28'], ['Miscellaneous', 9, 'misc']],
  mono: [['Project 2', 6, 'project-2'], ['Project 6', 7, 'project-6'], ['Project 7', 11, 'project-7'], ['Project 8', 4, 'project-8'], ['Project 9', 6, 'project-9'], ['Project 10', 10, 'project-10'], ['Project 11', 3, 'project-11'], ['Project 13', 13, 'project-13'], ['Project 14', 7, 'project-14'], ['Project 15', 4, 'project-15'], ['Project 17', 3, 'project-17'], ['Project 18', 16, 'project-18'], ['Project 19', 16, 'project-19'], ['Project 20', 16, 'project-20'], ['Project 21', 2, 'project-21'], ['Project 22', 5, 'project-22'], ['Miscellaneous', 1, 'misc']]
};
const projectSlug = label => label.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
const archiveCatalog = (folder, prefix, groups) => {
  return groups.flatMap(([group, count, groupSlug]) => Array.from({ length: count }, (_, groupIndex) => {
    const number = String(groupIndex + 1).padStart(3, '0');
    return {
      group,
      slug: groupSlug,
      src: `media/archive-v11/${folder}/${groupSlug}/${number}.jpg`,
...
if (projectPage) {
  ...
  const groups = projectPage.groups.map(([label, count, groupSlug]) => ({ label, count, slug: groupSlug, first: { src: `media/archive-v11/${projectPage.folder}/${groupSlug}/preview.jpg` } }));
  ...
  if (selectedProject) {
    ...
    gallery.innerHTML = catalog.filter(image => image.slug === selectedProject.slug).map(image => `<img class="fade" data-group="${image.group}" src="${image.src}" alt="${image.alt}">`).join('');
  } else {
    gallery?.remove();
    selection.innerHTML = `<div class="project-card-grid">${groups.map(group => `<article class="project-card reveal"><a href="${current}?project=${group.slug}"><img class="fade" src="${group.first.src}" alt="Preview of ${group.label}"><div><small>${group.count} images</small><h2>${group.label}</h2><span>Open project</span></div></a></article>`).join('')}</div>`;
    const back = document.querySelector('.collection-back');
...
if (current === 'filmmaking.html') {
  ...
  if (bts) bts.innerHTML = Array.from({ length: 24 }, (_, index) => `<img class="fade" src="media/archive/bts/bts-${String(index + 1).padStart(3, '0')}.jpg" alt="Behind the scenes photograph ${index + 1}">`).join('');
  ...
  const upcomingButtons = [...document.querySelectorAll('.upcoming-open')];
  if (upcomingButtons.length) {
    document.body.insertAdjacentHTML('beforeend', `<dialog class="upcoming-lightbox" aria-label="Upcoming title image"><button class="upcoming-close" type="button">Close</button><figure><img src="" alt=""><figcaption></figcaption></figure></dialog>`);
    const lightbox = document.querySelector('.upcoming-lightbox');
    const fullImage = lightbox.querySelector('img');
    const caption = lightbox.querySelector('figcaption');
    upcomingButtons.forEach(button => button.addEventListener('click', () => {
      fullImage.src = button.dataset.full;
      fullImage.alt = `${button.dataset.title} full-size title image`;
      caption.textContent = button.dataset.title;
      lightbox.showModal();
    }));
    lightbox.querySelector('.upcoming-close').addEventListener('click', () => lightbox.close());
...
document.querySelectorAll('.gallery').forEach(gallery => {
  ...
  const groupNavigation = groups.length > 1 ? `<div class="gallery-groups" aria-label="Project folders">${groups.map((group, index) => `<button class="${index === 0 ? 'active' : ''}" data-group="${group}">${group}</button>`).join('')}</div>` : '';
...
if (projectPage && selectedProject) {
  ...
  anchor?.insertAdjacentHTML('afterend', `<nav class="collection-switcher" aria-label="Project navigation"><a href="${current}?project=${previous.slug}">Previous project</a><a href="${current}">All ${projectPage.label} projects</a><a href="${current}?project=${next.slug}">Next project</a></nav>`);
}
